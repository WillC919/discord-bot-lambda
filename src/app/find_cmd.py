...
def make_player_data_embed_base(player_data: dict, map: str, mode: str):
    # retrieveCache = False
    if not player_data['success']:
        if player_data['cause'] == "You have already looked up this name recently":
            ...
            return make_error_embed(501, f"**{reformat_username(player_name)}**'s dataset found. Unable to retrieve it. Try again in 1 min!")
        elif player_data['cause'] == "No Such Players":
            return make_error_embed(201, f"**{reformat_username(player_name)}** does not exist in Hypixel's Databases!")
        elif player_data['cause'] == "No Arcade Data":
            return make_error_embed(202, f"**{reformat_username(player_name)}** has not played any Arcade Games!")
...
def make_player_data_embed(player_data: dict, map: str, mode: list, colors: list):
    ...
    if map == "General":
        embed_data = [{
            "title": reformat_username(player_data['Name']),
        ...
        if mode[0] == "General":
            ...
            for m in mode:     
                embed = {
                    ...
                    "color": colors[mode.index(m)],  # Note that the color should be an integer value
                    "thumbnail": { "url": f"{SKIN_LINK}{player_data['UUID']}"},
                    "fields": [
